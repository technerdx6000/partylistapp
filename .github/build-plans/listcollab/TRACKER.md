...
| 2 | Turborepo, TypeScript and test infrastructure | 🔄 In Progress | 2 / 9 |
...
| 2 – Foundation | 2.4 | Create `packages/shared` with Zod + `calculateCoverage` | 🔄 In Progress | Creating the shared TypeScript package that will hold current schemas, inferred types, and the first shared coverage helper |
| 2 – Foundation | 2.5 | Convert the API to TypeScript (no restructuring) | 🔄 In Progress | `apps/api` now builds from `server.ts`, `config/database.ts`, and typed route modules under `tsc`; Docker runs compiled output from `dist/` and the container still serves `/api/health` and `/api/required-items` |
| 2 – Foundation | 2.6 | Convert the web app to TypeScript (minimum typing only) | ⬜ Not Started | Do not restructure `App.tsx` — Phase 5 deletes it |
